...
function errorlytics_params($path, $key) {
    $params = array(
        'error[http_host]' => $_SERVER['HTTP_HOST'],
    ...
    $params['signature'] = sha1($client_occurred_at . $path . $key);
    $params['error[fake]'] = 'false';
    $params['format'] = 'xml';
    return $params;
}

function errorlytics_post($url, $key, $account_id, $website_id) {
...
function errorlytics_404() {
    if (is_404()) {
        ...
        if ($url) $url = chop($url, '/');
        $key = get_option(ERRORLYTICS_SECRET_KEY_OPT_NAME);
        ...
        if (!$url || !$key || !$account_id || !$website_id) return;

        $response = errorlytics_post($url, $key, $account_id, $website_id);
